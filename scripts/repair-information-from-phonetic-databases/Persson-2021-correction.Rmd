...
d.vowels.swe <- read.csv('../../data/phonetic vowel statistics/Published databases/Persson-2021/raw-formant-data/vowel_formants_swe_summary_native.txt', sep = "\t", fileEncoding = "UTF-16BE") %>%
  mutate(
...
# Repairing variables
We add information on talker age and language background (collected manually on paper forms when recording).
...
d.vowels.swe %<>%
  mutate(
    Age = case_when(Talker == "SW_001" ~ 30,
                    ...
                    Talker == "SW_024" ~ 22),
    L1 = case_when(
      Talker == "SW_013" ~ "Swedish-Persian",
      Talker == "SW_020" ~ "Swedish-Korean",
      Talker == "SW_021" ~ "Swedish-French",
      Talker == "SW_022" ~ "Swedish-Spanish",
      TRUE ~ "Swedish")) %>%
  relocate(c(Age, L1), .after = Talker)
```

Some vowels were misidentified/misproduced by the talkers. This was e.g. the case for several of the /hodd/ tokens. After listening through all sound files, we identified these tokens and add a column to the corrected data. Some talkers only did 5 recordings of each vowel in a first session. They were called back, some returned but some did not. We add a column indicating the number of recordings per talker.
...
d.vowels.swe %<>%
  # focus on F1-F2
  select(
    Talker, Age, L1, Vowel, Transcribed_vowel, Word, Token, Trial, F0_gm, F0, Duration, SR, Quantity,
...
d.vowels.swe %<>%
  select(Talker, Age, L1, Vowel, Transcribed_vowel, Word, Token, Trial, Location, F1, F2, F3, F0_gm, F0, Duration, SR, Quantity) %>%
  left_join(
  ...
  select(Talker, Age, L1, Vowel, Transcribed_vowel,  Word, Token, Trial, Location, F0_gm, F0, Duration, SR, F1, F2, F3, Quantity) %>%
...
d.vowels.swe %<>%
  ...
  select(Talker, Age, L1, Vowel, Transcribed_vowel,  Word, Token, Trial, Location, F0_gm, F0, Duration, SR, F1, F2, F3, height, backness, Quantity)
...
#     group_by(Talker, Vowel) %>%
#     mutate(
#       F0_half = ifelse(F0.mean < (F0_gm / 2), TRUE, FALSE),
#       F0_double = ifelse(F0.mean > (F0_gm * 2), TRUE, FALSE)) %>%
#     filter(F0_half == TRUE | F0_double == TRUE | is.na(F0.mean)) %>%
#     select(Talker, Vowel, Token, Trial, Location, Quantity, F1, F2, F0.mean),
...
d.vowels.swe %<>%
  select(Talker, Age, L1, Vowel, Transcribed_vowel, Word, Token, Trial, Location, F1, F2, F3, F0_gm, F0, Duration, SR, Quantity, Unreliable_measurement) %>%
  left_join(
  ...
  select(Talker, Age, L1, Vowel, Transcribed_vowel,  Word, Token, Trial, Location, F0_gm, F0, Duration, SR, F1, F2, F3, Quantity, Unreliable_measurement)
