...
#Looking at the repaired male talker vowels, there were clearly several AA1 tokens by talker SW_032 that haven't been targeted by the outlier detection approach, hence, write a new repair file that includes all AA1
# write.csv(
#   d.vowels.swe.male %>%
#     # Mutate back to arpabet for csv readability
#     mutate(Vowel = factor(
...
#         levels.vowel.SweFA),
#       levels = levels.vowel.SweFA)) %>%
#     filter(Talker == "SW_032" & Vowel == "aa1") %>%
#     select(Talker, Vowel, Token, Trial, Location, Quantity, F1, F2, F3),
#   file = "../../data/phonetic vowel statistics/Published databases/Persson-2021/corrected-vowel-data/Persson_2021_L1_male_SW032_to_repair.csv",
#   row.names = FALSE, quote = FALSE)

# Read in corrected values for talker SW032
d.vowels.swe.male %<>%
  select(Talker, Age, Gender, Vowel, Transcribed_vowel, Word, Token, Trial, Location, F1, F2, F3, F0_gm, F0, Duration, SR, Quantity, Unreliable_measurement) %>%
  # add the corrected F1, F2
  left_join(
    read_csv('../../data/phonetic vowel statistics/Published databases/Persson-2021/corrected-vowel-data/Persson_2021_L1_male_SW032_repaired.csv') %>%
