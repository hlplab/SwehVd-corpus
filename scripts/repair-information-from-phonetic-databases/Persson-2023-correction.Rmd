...
d.vowels.swe.male <- read.csv('../../data/phonetic vowel statistics/Published databases/Persson-2023/raw-formant-data/vowel_formants_swe_summary_native_male.txt', sep = "\t", fileEncoding = "UTF-16BE") %>%
  rbind(read.csv('../../data/phonetic vowel statistics/Published databases/Persson-2023/raw-formant-data/vowel_formants_swe_summary_SW042.txt', sep = "\t", fileEncoding = "UTF-16BE")) %>%
  mutate(
    Trial = as.numeric(gsub("^.*\\_([0-9]+)$", "\\1", Token)),
    F0.mean = ifelse(F0.mean == "--undefined--", NA, as.numeric(F0.mean)),
    Vowel = factor(
...
# Write a to-repair file with missing F0 values and instances of doubling and halving
# write.csv(
#   d.vowels.swe.male %>%
#     # Mutate back to arpabet for csv readability
#     mutate(Vowel = factor(
